...
if __name__ == '__main__':
    ...
    # print(cc3df)
    dist_vet = np.sqrt(np.sum((cc3df[-1,:] - cc3df[0,:])**2))
    
    vel_med = dist_vet / (len(cc3df) * (1/freq)) * 3.6
    plt.title(f'Speed (Max = {np.round(max(vels),2)} km/h ; Mean = {np.round(vel_med)}); Angles (azi = {np.round(azi,1)}, elev. = {np.round(elev,1)})')
    plt.savefig('grafico_reconstrucao.png')

    # import pdb; pdb.set_trace() 
    resultado = list(np.append(vels, [azi, elev, vel_med]))
    np.savetxt(str(sys.argv[6])+'_result.txt', resultado, fmt='%.10f')
    print(f'Mean Speed = {vel_med}')
    print('\n')
